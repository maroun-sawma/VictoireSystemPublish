...
function Service_Call_Completed(i_Response) {
    try {
        ...
        switch (_Service_Method) {
            case "Edit_MobileAjialReceiptPaymentData":
                Edit_MobileAjialReceiptPaymentData_Completed(i_Response);
                break;
        }
    }
    catch (e) {
...
function Btn_Pay_Policy_Click() {
    try {
        ...
        //alert(_Params);    
        _Service_Method = "Edit_MobileAjialReceiptPaymentData";
        _Async = true;
        CallService();
        /* ---------------- */
    }
...
function Edit_MobileAjialReceiptPaymentData_Completed(i_Response) {
    try {
        debugger;
        if (i_Response.My_Result != null) {
            ...
            $("#txtIndex").val(i_Response.My_Result.RECEIPTS_ID);
            $("#txthttp").val('https://online.victoire.com.lb/MobileAjialReceiptReturnPayment.aspx');
            //$("#txthttp").val('http://localhost:4160/MobileAjialReceiptReturnPayment.aspx');
            ...
            $("#Frm_ReceiptAjialNetCommerce").submit();
            return false;
        }
        else {
        ...
        alert("Edit_MobileAjialReceiptPaymentData: " + e.message);
    }
}
